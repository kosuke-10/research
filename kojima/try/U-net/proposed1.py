...
def calculate_circularity(epoch, train_set, palette, index_void=None):
    ...
    for i in range(len(train_set)):
            train_image = get_imageset(train_set[i], palette, index_void) # Numpy → pillow 
...
def train(parser):
    ...
    with tf.control_dependencies(update_ops):
        train_step = tf.train.AdamOptimizer(1e-3).minimize(tf.reduce_mean([cross_entropy, circularity]))
    ...
    if parser.resume and os.path.exists('./model/deploy.ckpt'):
        print("Restoring the model from ./model/deploy.ckpt ...", end="", flush=True)
    ...
    for epoch in range(epochs):
        ...
        for batch in train(batch_size=batch_size, augment=is_augment):
            ...
            sess.run(train_step, feed_dict={circularity: passed_circularity, model_unet.inputs: inputs, model_unet.teacher: teacher, model_unet.is_training: True})
        ...
        if epoch % 1 == 0:
            ...
            if loss_test<min_loss_test and accuracy_test>max_accuracy_test:
...
def get_parser():
    ...
    parser.add_argument('-ru', '--resume', action='store_true',help='checkpoint and epoch data ')
