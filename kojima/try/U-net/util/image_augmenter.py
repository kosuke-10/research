...
class ImageAugmenter:
    ...
    def __init__(self, size, class_count):
        ...
        self._ph_original = tf.placeholder(tf.float32, [size[0], size[1], 3])  #tf.placeholder(tf.float32, [size[0], size[1], 3])
    ...
    def augment(self, image_in, image_out, method=None):
        ...
        return self._sess.run([op["original"], op["segmented"]], feed_dict={self._ph_original: image_in,self._ph_segmented: image_out})
