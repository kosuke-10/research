...
class UNet:
    ...
    def create_model(size, l2_reg):
        inputs = tf.compat.v1.placeholder(tf.float32, [None, size[0], size[1], 3], name="input")
        teacher = tf.compat.v1.placeholder(tf.float32, [None, size[0], size[1], len(ld.DataSet.CATEGORY)])
    ...
    def conv(inputs, filters, kernel_size=[3, 3], activation=tf.nn.relu, l2_reg_scale=None, batchnorm_istraining=None, name=None):
        if l2_reg_scale is None:
            ...
            regularizer =tf.keras.regularizers.l2(l2_reg_scale)
    ...
    def bn(inputs, is_training):
        normalized = tf.compat.v1.layers.batch_normalization(
            inputs=inputs,
            axis=-1,
            momentum=0.9,
            epsilon=0.001,
    ...
    @staticmethod
    def pool(inputs):
        pooled = tf.compat.v1.layers.max_pooling2d(inputs=inputs, pool_size=[2, 2], strides=2)
        return pooled
    ...
    def conv_transpose(inputs, filters, l2_reg_scale=None):
        if l2_reg_scale is None:
            ...
            # regularizer = tf.contrib.compat.v1.layers.l2_regularizer(scale=l2_reg_scale)
            regularizer =tf.keras.regularizers.l2(l2_reg_scale)
        conved = tf.compat.v1.layers.conv2d_transpose(
            inputs=inputs,
            filters=filters,
            strides=[2, 2],
            kernel_size=[2, 2],
